...
s = imread('Images/decoloration.png');
...
c = imread('Images/decoloration.png');
...
c = rgb2lab(c);
c(:,:,2) = zeros(nb_lignes_c, nb_colonnes_c);
c(:,:,3) = zeros(nb_lignes_c, nb_colonnes_c);
s = rgb2lab(s);
...
u = lab2rgb(c);
interieur = find(p>0);
u(i_p_min:i_p_max,j_p_min:j_p_max,:) = lab2rgb(collage(r,s,interieur,false));
hold off;
imagesc(u);
axis image off;
