...
u_restored = reshape(u, [nb_lignes nb_colonnes]);
u_restored_norm = uint8(255 * max(0, min(1, u_restored / u_max)));
imwrite(u_restored_norm, 'Resultats/image_restauree.png');

diff_image = (reshape(u_0 - u, [nb_lignes nb_colonnes])) / u_max;
diff_image_disp = uint8(255 * max(0, min(1, (diff_image + 1) / 2)));
imwrite(diff_image_disp, 'Resultats/soustraction_solution.png');
