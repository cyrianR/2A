...
% Lecture de l'image :
u_0 = double(imread('Images/debruitage_bad-1.jpg'));
...
u_restored = reshape(u_k, [nb_lignes nb_colonnes]);
u_restored_norm = uint8(255 * max(0, min(1, u_restored / u_max)));
imwrite(u_restored_norm, 'Resultats/var_totale_restauree.png');

diff_image = (reshape(u_0 - u_k, [nb_lignes nb_colonnes])) / u_max;
diff_image_disp = uint8(255 * max(0, min(1, (diff_image + 1) / 2)));
imwrite(diff_image_disp, 'Resultats/var_totale_soustraction.png');
