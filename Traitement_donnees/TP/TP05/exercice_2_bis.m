...
% Calcul de W à partir d'une segmentation de la couleur jaune
D = u_0(:,3);
D(D>0) = 1;
...
imwrite(u_restored_norm, 'Resultats/grenouille_restauree.png');

diff_image = (reshape(u_0 - u_k, [nb_lignes nb_colonnes nb_canaux])) / u_max;
diff_image_disp = uint8(255 * max(0, min(1, (diff_image + 1) / 2)));
imwrite(diff_image_disp, 'Resultats/grenouille_soustraction.png');
