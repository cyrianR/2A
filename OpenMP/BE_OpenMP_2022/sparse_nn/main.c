...
void parallel_nn_loops(layer *layers, int n, int L){
  ...
  #pragma omp parallel private(s,i,j,l)
  {
    for(l=0; l<L-1; l++){
      /* printf("layer %2d  m:%2d\n",l,layers[l].m); */
      #pragma omp for
  ...
  #pragma omp parallel private(l,s,i,j)
  {
    #pragma omp single
    {
      for(l=0; l<L-1; l++){
        /* printf("layer %2d  m:%2d\n",l,layers[l].m); */
        for(s=0; s<layers[l].m; s++){
          //#pragma omp task depend(in:layers[l].syn[s]) depend(out:i)
          i = layers[l].syn[s].i;
          //#pragma omp task depend(in:layers[l].syn[s]) depend(out:j)
          j = layers[l].syn[s].j;
          /* printf("layer %2d  i:%2d  j:%2d\n",l,i,j); */
          #pragma omp task depend(in:layers[l].neu[i], layers[l].syn[s]) depend(inout:layers[l+1].neu[j])
          layers[l+1].neu[j].nv += update(layers[l].neu[i].nv, layers[l].syn[s].sv);
        }
      }
    }
