...
let retrait mot (Trie(arbre, decompose, recompose)) = 
  Trie(nettoie (retrait_arbre (decompose mot) arbre), decompose, recompose)
...
let trie_dico (Trie(arbre, decompose, recompose)) = 
  List.map (fun l -> recompose l) (to_List arbre)
...
(*   résultat   : aucun                                                       *)
(******************************************************************************)
let affiche p abr = failwith "bug?" (*let (Trie(arbre, decompose, recompose)) = abr in
  let _ = List.map (fun li -> p (recompose li); print_newline) (trie_dico abr) in ();;*)