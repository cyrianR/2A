...
let tri_insertion ordre l =
  List.fold_right(fun t tq -> insert ordre t tq) l []
...
let rec scinde l =
  ...
  | [] -> ([], [])
  | [_] -> (l, [])
  | t1::t2::q -> (t1::fst(scinde q), t2::snd(scinde q))

let rec scinde2 l =
  match l with
  | [] -> ([], [])
  | [_] -> (l, [])
  | t1::t2::q -> let l1,l2 = scinde2 q in 
  (t1::l1, t2::l2)
...
let ordre_prenom q1 q2 = let (_,_,_,n) = q1 and (_,_,_,m) = q2 in n > m

let tri1 = tri_fusion ordre_prenom listStat;;
