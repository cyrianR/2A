...
public class PhiloCondition implements StrategiePhilo {
    ...
    class Scheduler implements Runnable {
        public void run() {
            ...
            for (int i = 0; i < nbPhilosophes; i++) {
                EtatPhilosophe etatPhiloDroite = etat[Main.PhiloDroite(i)];
                EtatPhilosophe etatPhiloGauche = etat[Main.PhiloGauche(i)];
                gchan[i] = new GuardedChannel<>(entrer[i], () -> (etatPhiloDroite != EtatPhilosophe.Mange && etatPhiloGauche != EtatPhilosophe.Mange));
            }
            for (int i = nbPhilosophes; i < 2*nbPhilosophes; i++) {
                gchan[i] = new GuardedChannel<>(sortir[i], Predicate::True);
            }
            ...
            while (true) {
                PhiloCondition.ChannelId chanId = alt.select();
