...
public class PhiloCondition implements StrategiePhilo {
    ...
    class Scheduler implements Runnable {
        ...
        private EtatPhilosophe etat[] = new EtatPhilosophe[nbPhilosophes];
        ...
        public void run() {
            ...
            for (int i = 0; i<nbPhilosophes; i++) {
                final int j = i;
                gchan[i*2] = new GuardedChannel<>(entrer[i], () -> (
                    (etat[Main.PhiloGauche(j)] != EtatPhilosophe.Mange) &&
                    (etat[Main.PhiloDroite(j)] != EtatPhilosophe.Mange)) );
                gchan[i*2+1] = new GuardedChannel<>(sortir[i], Predicate::True);
            }
            ...
            while (true) {
                ChannelId selected = alt.select();
                if (selected.entrer) {
                    entrer[selected.num].read();
                    etat[selected.num]=EtatPhilosophe.Mange;
                } else {
                    etat[selected.num] = EtatPhilosophe.Demande;
                    sortir[selected.num].read();
                    etat[selected.num] = EtatPhilosophe.Pense;
                }
            }
        }
    } // class Scheduler
