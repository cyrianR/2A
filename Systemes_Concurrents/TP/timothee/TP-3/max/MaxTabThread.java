...
public class MaxTabThread {
    ...
    private static int computeMax(int[] array, int numberOfThreads) throws InterruptedException {
        ...
        List<PartialMax> jobs = new LinkedList<PartialMax>();
        for (int i = 0; i <= numberOfThreads-1; i++ ) {
            jobs.add(new PartialMax(array, i*taskSize, (i+1)*taskSize, results));
        }
        jobs.add(new PartialMax(array, taskSize*(numberOfThreads -1), Math.min(taskSize*numberOfThreads,array.length), results));
