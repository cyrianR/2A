...
public class MaxTabPool {
    ...
    private static int computeMax(ExecutorService executor, int[] array, int numberOfTasks) throws InterruptedException, ExecutionException {
        ...
        for (int i = 0; i <= numberOfTasks-1; i++ ) {
            results.add(executor.submit(new PartialMax(array, i*taskSize, (i+1)*taskSize)));
        }
        results.add(executor.submit(new PartialMax(array, taskSize*(numberOfTasks -1), Math.min(taskSize*numberOfTasks,array.length))));
        

        // Récupérer les résultats et les fusionner
        int max = results.get(0).get();
        for (int i = 0; i < numberOfTasks; i++) {
            max = Math.max(results.get(i).get(), max);
        }
        /* XXXX À COMPLÉTER XXXX */
